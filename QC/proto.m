...
axes2 = axes('Parent', Anneal_fig, ...
    ...
    'HandleVisibility','callback', ...
    'Position',[0.51 0.1 0.21 0.4]);


%panel2 = uipanel(Anneal_fig, ...
%    'Position',[0.51 0.1 0.21 0.4],...
%    'BorderWidth',1,...
%    'BackgroundColor','white');

%panel3 = uipanel(Anneal_fig, ...
%    'Position',[0.75 0.1 0.21 0.4],...
%    'BorderWidth',1,...
%    'BackgroundColor','white');

%panel_focal = uipanel(Anneal_fig, ...
%    'Position',[0.84 0.6 0.14 0.21],...
%    'BorderWidth',1,...
%    'BackgroundColor','white');
...
axes3 = axes( 'Parent', Anneal_fig, ...
    ...
    'Box', 'on', ...
    'HandleVisibility','callback', ...
    'Position',[0.510 0.525 0.21 0.025]);
...
axes6 = axes( 'Parent', Anneal_fig, ...
    ...
    'Box', 'on', ...
    'HandleVisibility','callback', ...
    'Position',[0.75 0.525 0.21 0.025]);
...
data.Data_Dir = '/Users/leokan/wrk/wrk_alldata';  % default data directory
...
function DisplayMap
        ...
        m_proj('mercator','lon',[-260,100],'lat',[-65,65]);
        m_coast('color','k');
...
[M,text] = DownloadGCMT(); %[mrr, mtt, mff, mrt, mrf, mtf]
...
function Radiobutton_trace
        ...
        for ii = 1:data.Ntraces
            ...
            data.rb(ii) = uicontrol( ...
              ...
              'Interruptible', 'On', ...
              'Position', [0.03 0.8-(row -1 +1.75)*(0.7/(data.NbDisp+2)) 0.02 0.02]);
        end
        guidata(Anneal_fig,data)
end
% ----------------------------------------------------------------------
function Radiobutton_radial
        ...
        for ii = 1:data.Ntraces
            ...
            data.rb2(ii) = uicontrol( ...
              ...
              'Interruptible', 'On', ...
              'Position', [0.26 0.8-(row -1 +1.75)*(0.7/(data.NbDisp+2)) 0.02 0.02]);
        end
        guidata(Anneal_fig,data)
end
% ----------------------------------------------------------------------
    ...
    function Display_Time_Residuals
        ...
        for i = 1:Ntraces            
            if SelectionZ(i) == 1
                ...
                Sta(count,1) = data.SigFlZ(i).HdrData.STLO;
                Sta(count,2) = data.SigFlZ(i).HdrData.STLA;
                Sta(count,3) = data.residu(i);
                staname =  convertCharsToStrings(data.SigFlZ(i).HdrData.KSTNM);
