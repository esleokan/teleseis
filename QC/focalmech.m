...
if true
    if vT(3) >= 0
        plot(centerX+sE*vT(2),centerY+sN*vT(1),'ro','MarkerFaceColor',[1 1 1],'MarkerSize',12,'MarkerEdgeColor','black')
        text(centerX+sE*vT(2),centerY+sN*vT(1),'T','color',[0 0 0],'FontSize',8,'HorizontalAlignment','Center','FontWeight','Bold')
    else
        plot(centerX-sE*vT(2),centerY-sN*vT(1),'ro','MarkerFaceColor',[1 1 1],'MarkerSize',12,'MarkerEdgeColor','black')
        text(centerX-sE*vT(2),centerY-sN*vT(1),'T','color',[0 0 0],'FontSize',8,'HorizontalAlignment','Center','FontWeight','Bold')
    end
    ...
    %end
    if vP(3) >= 0
        plot(centerX+sE*vP(2),centerY+sN*vP(1),'ro','MarkerFaceColor',[0 0 0],'MarkerSize',12,'MarkerEdgeColor','black')
        text(centerX+sE*vP(2),centerY+sN*vP(1),'P','color',[1 1 1],'FontSize',8,'HorizontalAlignment','Center','FontWeight','Bold')
    else
...
if plot_piecring == true
    tt = tauppath('Model','prem','Evdp',depth,'Degrees',deg,'Phases',phase);
    takeoff_z = tt(1).path.depth(2) - tt(1).path.depth(1);
    takeoff_x = deg2km(tt(1).path.distance(2) - tt(1).path.distance(1));
    takeoff_angle = atand(takeoff_x/takeoff_z);
    
    %stereographic projection
    length_on_equatorial_plane = tand(takeoff_angle/2);
    
    x_piecring = (length_on_equatorial_plane)*cos(deg2rad(azi)-pi/2);
    y_piecring = (length_on_equatorial_plane)*sin(deg2rad(azi)+pi/2);

    plot(x_piecring, y_piecring,'ro','MarkerFaceColor',[0 0 1],'MarkerSize',12,'MarkerEdgeColor','black')

end

% add text to beachball
if ~isempty(ctitle)
    text(-1.3,-0.8,ctitle,'HorizontalAlignment','left','FontSize',fsize);
end
